...
/**
 * 안내 스킬 한 벌을 고객 홈에 심는다 — `~/.claude/skills/reborn-claudekit/`.
 *
 * ★왜 터미널 한 줄로 안 끝내나
...
function 안내스킬심기() {
  ...
  try {
    ...
    writeFileSync(join(폴더, "SKILL.md"), readFileSync(원본, "utf8"), "utf8");
    return true;
  } catch {
    // 권한이 없거나 홈을 못 찾는 환경이 있다. **설치를 실패로 만들지 않는다** —
    // 다섯 개를 깔아 주는 게 이 도구의 일이고 이건 덤이다.
    return false;
  }
}
 ...
 *   ③ **파일을 남기지 않는다.** 화면에 지나가는 글자일 뿐이고, 설치기는 고객
 *      디스크에 아무것도 쓰지 않는다. 몰래 심는 게 아니라 눈앞에 보이는 것이다.
 */
if (된것 > 0) {
  안내스킬심기();
